...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        sanctum = root / "_bmad" / "memory" / "agent-it-infra-expert"
        if r.returncode != 0 or not (sanctum / "PERSONA.md").is_file():
            ...
            for code in ("DX", "SSH", "NET", "CLD", "CTR", "SEC"):
                if f"[{code}]" not in caps:
                    print("FAIL: missing cap", code)
                    fails += 1
                    break
            else:
                print("PASS: init scaffolds sanctum with 6 caps")
            ...
            if "Rex Wire" not in persona:
                print("FAIL: PERSONA missing Rex Wire")
                fails += 1
            else:
                print("PASS: PERSONA seeded Rex Wire")
            r2 = subprocess.run(
                [sys.executable, str(INIT), str(root), str(SKILL)],
                capture_output=True,
                text=True,
