...
**Consiglio (`bmad-party-mode --non-interactive`, gruppo `super-esperti`) — tre goal:** **G1 lettura** (superficie · `activity` · `register` · perimetro · stack · precedenze: scioglie ogni ambiguità che sarebbe stata una domanda), **G2 kernel + `slice_plan`**, **G3 approvazione** del deliverable — approvabile se soddisfa tutte le richieste dei documenti BMAD + craft-rules. Parla chi ha giurisdizione (privacy · fisco · legale · infra · WordPress · motion); **il craft non si vota**. Un consiglio che restituisce una domanda ha fallito il goal: si rilancia il goal, non si gira la domanda.
