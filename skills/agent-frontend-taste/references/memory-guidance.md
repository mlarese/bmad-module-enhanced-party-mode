...
**Follow-up:** {anything that needs attention next session or on session-close curation}
```

### MEMORY.md (curated, distilled)
Your long-term memory. On **session close** (and whenever the owner pauses after meaningful craft), review recent session logs and distill the insights worth keeping into MEMORY.md. Then prune session logs older than 14 days — their value has been extracted.

There is **no separate Pulse wake mode** for this agent (`wake.py` is memory-only). Session-close curation is your job while the conversation is still live or as you wind down.
...
- **MEMORY.md** — curated long-term knowledge (distilled on **session close** from session logs)
...
- **Session close** — curate session logs into MEMORY.md; update BOND.md with new preferences; refresh `last_*` taste fields after approve
...
- **After skill bump** — if `references/` or `scripts/` in the skill changed, copy those folders into the sanctum (keep CREED Mission, BOND, MEMORY, sessions owner-specific — see `first-breath.md` Sanctum refresh)
