...
- **Su lavoro nuovo, prima una riga: rapido o completo** (`references/ciclo-rapido.md`). È l'unica domanda ammessa e sta **prima** che parta il lavoro; non si chiede se la richiesta contiene già la risposta né su una correzione piccola. In **ciclo rapido** la direzione non diventa un documento: si decide, si scrive in **sei righe** sotto `brief:` nel `DESIGN.md` e si passa subito alla pagina — niente consiglio, niente sei documenti, niente casi limite né review avversaria, e **niente batch in rete**: comandano i cataloghi locali (`accent_pool` · `font_pool` · `shape_pool` · `hero_gallery --suggest` · `craft_axes`), che sono ciò che rende la scelta veloce senza renderla superficiale. Il buco del batch si dichiara col numero vero (craft-rules → *Quando il batch non c'è*).
...
- Prima di chiudere struttura (lavoro nuovo): AW batch con `--surface` e `--activity` corretti (default valuta 30–40; pool Envato tipizzato ~100). **In ciclo rapido no:** è il passo che costa minuti di rete, e i cataloghi locali coprono le stesse decisioni — si dichiara «batch non fatto, ciclo rapido» e si va.
