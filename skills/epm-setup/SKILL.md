---
name: "epm-setup"
description: Sets up Enhanced Party Mode module in a project. Use when the user requests to 'install epm module', 'configure Enhanced Party Mode', or 'setup Enhanced Party Mode'.
---
...
3. Check for per-module configuration at `{project-root}/_bmad/epm/config.yaml` and `{project-root}/_bmad/core/config.yaml`. If either file exists:
...
```bash
python3 ./scripts/cleanup-legacy.py --bmad-dir "{project-root}/_bmad" --module-code epm --also-remove _config --skills-dir "{project-root}/.claude/skills"
```
