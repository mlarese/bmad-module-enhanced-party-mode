...
def main() -> int:
    ...
    missing = [n for n in REQUIRED_FOR_WAKING if not (sanctum / n).is_file()]
    if pulse and not (sanctum / "PULSE.md").is_file():
        missing.append("PULSE.md")

    print("MODE: PULSE" if pulse else "MODE: WAKING")
    print(f"Sanctum: {sanctum}")
    if missing:
        print(f"STRUCTURE_WARN missing={','.join(missing)}")
